...
import {
  vaccinationStateDetails
} from "../../admin/AdminDashBoard/vaccinationStateDetails";
...
export const ViewAdmin = ({history}) => {
  ...
  const handleChange = (state) =>
      (_, isExpanded) => { setExpanded(isExpanded ? state : false); };

  useEffect(() => {
    let url = `${process.env.REACT_APP_SERVER}/su/admins/${
        adminEmail}/registered_patients?`;
  ...
  onChange = {(e) => setLastNumDays(e.target.value)} className =
      ...
      Days</span>
            ...
            data={
    {
      labels: Object.keys(scheduledOrVaccinatedPatients).map((day) => day),
          datasets: [
            {
              label : "#patients vaccinated",
              data :
                  Object.keys(scheduledOrVaccinatedPatients)
                      .map((day) =>
                               registeredPatients["vaccinated"][day]?.length ||
                               0),
              borderColor : "#00e600",
            ...
            },
            {
              ...
              data : Object.keys(registeredPatients["scheduled"])
                         .map((day) =>
                                  registeredPatients["scheduled"][day]?.length |
                                  0),
              borderColor : "#ff4d4d",
...
Days<
    ...
    nbsp;
Days</span>
        </Typography> {["scheduled", "notScheduled", "vaccinated"].map((state) => (
          <Accordion
  key =
      {state} expanded = {expanded === state} onChange = {handleChange(state)} > <AccordionSummary
                                                                                      expandIcon =
                                                                                          {
                                                                                            <ExpandMoreIcon />
                                                                                          }><Typography
                                                                                                 className = {classes
                                                                                                                  .accordionHeading}>{vaccinationStateDetails
                                                                                                                                          [state]
                                                                                                                                              .name}</Typography>
  ...
  paddingBottom: " 3vh" }}>
        ...
        >
          <Marker latitude={lat} longitude={lng}>
            <button className="marker-btn">
              <img src="/vaccinationIcon
                            .webp " alt=" Vaccination center " />
                        < /button>
          </Marker> {Object.entries(scheduledOrVaccinatedPatients).map(
            ([_date, patients], idx) => (
      ...
      const {
        ...
        isVaccinated,
                  ...
                  onClick = {() => setSelectedPatient({...patient})} > < img
                          src={
                            isVaccinated
        ...
        firstName + " " + lastName}
          ...
          {selectedPatient && (
            <Popup
      latitude = {selectedPatient.location.coordinates[0]} longitude =
          {selectedPatient.location.coordinates[1]} onClose =
              {() => setSelectedPatient(null)} className =
